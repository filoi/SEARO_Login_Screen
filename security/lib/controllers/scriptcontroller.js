...
var mdeids = [],
  mdenames = [];
var marr;
var mdegroupid, cdegroupid, mdegroupname, mdename, mdeid;
var mpeid, morgid;
...
Ext.onReady(function () {
  ...
  $.ajax({
    ...
    url:
      base + "dataElementGroups.json?fields=id,name,dataElementGroups[id,name]",
  ...
  $.ajax({
    ...
    dataType: "json",
    contentType: "application/json",
    url: base + "sqlViews/n0UQzBeeixt/data.json",
    success: function (data) {
      $.each(data.listGrid.rows, function (index, item) {
  ...
  $.ajax({
    ...
    url:
      base +
      "organisationUnitGroups.json?fields=id,code,name&level=3&order=code:asc",
    success: function (data) {
  ...
  $("#drop1").change(function () {
    ...
    $.ajax({
      ...
      url:
        ...
        orglevel +
        ".json?fields=id,name,organisationUnits[id,name]",
      success: function (data) {
  ...
  function toggleSelectAll(control) {
    ...
    if (control.data("allOptionIsSelected") != allOptionIsSelected) {
      ...
      if (
        allOptionIsSelected &&
        control.val().length != control.find("option").length
      ) {
        // All options were selected, user deselected one option
        // => unselect 'All' option
        control.selectpicker(
          "val",
          valuesOf(control.find("option:selected[value!=all]"))
        );
        allOptionIsSelected = false;
      } else if (
        !allOptionIsSelected &&
        control.val().length == control.find("option").length - 1
      ) {
...
function validatetable() {
  ...
  if (levelid == "base" || levelid == undefined || levelid == "") {
    ...
    (periodid = "" || periodid == "Please Select" || periodid == undefined)
  ) {
    alert("Please select Period");
    isValidated = "false";
    return;
  } else if (
    (indigid = "" || indigid == "Please Select" || indigid == undefined)
  ) {
...
function validatechart() {
  ...
  {
    window.open(
      "lib/controllers/highcharts1.html?cdegroupid=" +
        cdegroupid +
        "&cpeid=" +
        cpeid +
        "&cdegroupname=" +
...
function validatemap() {
  ...
  {
    window.open(
      "lib/controllers/map1.html?mdeid=" +
        mdeid +
        "&mpeid=" +
        mpeid +
        "&morgid=" +
        ";sCU6B2ZOTYr;GY9dLYENyOU;AFkkfLBwuHc;sQeCnfgIko4;n5AQFbBKbVA;vMK14bK6V4P;rV0O78pYETp;uu7IpfKO1IA;jqvWOjpUOLE;BYyRgdEoYDd;rY3Q1VZtZpg" +
        "&morgname=" +
        "Searo Region"
    );
  }
